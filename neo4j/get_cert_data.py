...
from neo4j.verify_cert_chain import verify_cert
from python_adb.read_json import read_json
from log_config import create_log


# file2app = read_json("../download_cache/file2app/file2app_apk_gupiaojijin_1.json")
...
def creat_complex_node(cert_data, cert_node_data):
    ...
    for node in complex_list:
        ...
        if node == 'Subject':
            ...
            # 先检查是否存在一个颁发者与证书的Issuer相同的主体节点
            if find_node(node_type, node2_property):
                # 如果颁发者已经被创建为主体，则直接创建关系，并为主体创建一个Issuer的类型
                node_relation = pack_relation('Issuer', 'name:"Issuer"')
                create_relation(pack_node(node_type, node2_property), cert_node_data, node_relation)
                add_type_label("Issuer", node2_property)
            else:
...
def cert_data_parse(cert_data, cert_type, x509_hash, cert_file_path=None):

    # 创建证书节点
    node1_type = 'Cert:' + cert_type
    serial_number = parse_serial_number(cert_data)
    # if serial_number == "0x268c12326885589b97cbac352b0cbdc":
    #     print(cert_file_path)
    if cert_type == '终端证书':
        app_type, app_name, file_name = parse_app_file(cert_file_path)
        # 证书的属性
        node1_property = '序列号:"' + str(serial_number) + '"' + ',' + 'Figure:"' + str(x509_hash) + '"' # 证书的节点和属性
        ...
        node1_property = '序列号:"' + str(serial_number) + '",Figure:"' + str(x509_hash) + '"'
        temp_node = find_node(node1_type, node1_property)
        if not temp_node:
            cypher_creat_cert_node(node1_type, node1_property)  # 创建证书的节点
        else:
...
def find_cert(file_path):
    try:
        ...
        if len(cert_chain) > 1:
            cert_chain = verify_cert(cert_chain)
        chain_length = cert_chain.__len__() - 1
        last_cert = cert_chain[chain_length]
        ...
        father_cert_figure = ""
        x509_hash = x509.digest("sha1").decode("utf-8")
        son_cert_figure = str(x509_hash)
        son_cert_serial_number = cert_data_parse(x509, '终端证书', son_cert_figure, file_path)
        for cert in cert_chain:
            x509 = parse_certificate(cert)
            x509_hash = x509.digest("sha1").decode("utf-8")
            # 检查得到的证书是不是根证书
            if x509.get_subject().CN == x509.get_issuer().CN:
                father_cert_figure = str(x509_hash)
                # 先检查是否存在该证书，如果已存在就不需要再搞别的，直接连接即可
                father_cert_serial_number = cert_data_parse(x509, '根证书', father_cert_figure)
                store_path = "../Scan_result/collect_root_cert/" + father_cert_serial_number + ".crt"
                store_cert(store_path, cert)
            else:
                father_cert_figure = str(x509_hash)
                father_cert_serial_number = cert_data_parse(x509, '中间证书', father_cert_figure)
                store_path = "../Scan_result/collect_mid_cert/" + father_cert_serial_number + ".crt"
                store_cert(store_path, cert)
            # if son_cert_figure == "D1:EB:23:A4:6D:17:D6:8F:D9:25:64:C2:F1:F1:60:17:64:D8:E3:49":
            #     print(file_path)
            upload_cert_chain(son_cert_serial_number, father_cert_serial_number, son_cert_figure, father_cert_figure)  # 链接证书链
            son_cert_figure = father_cert_figure
            father_cert_figure = ""
            # if son_cert_serial_number == "0x33af1e6a711a9a0bb2864b11d09fae5":
            #     print(file_path)
            son_cert_serial_number = father_cert_serial_number
            father_cert_serial_number = ""

    except Exception as e:
        log_driver.info("错误原因：" + str(e) + ",错误位置：" + file_path + ", 错误行数：" + str(e.__traceback__.tb_lineno))
        print(f"错误原因：{e},错误位置：{file_path}, 错误行数：{e.__traceback__.tb_lineno}")
    ...
    for _certs_path in os.listdir(certs_list_path):  # 软件类型
        ...
        for _cert in os.listdir(cert_path):  # 软件名称
            ...
            for cert_file in os.listdir(app_cert_file):  # 证书名称
                ...
                # all_file_path = "../Scan_result/certs\\gupiaojijin\\android_83\\121.28.129.9_18555_15.crt"
